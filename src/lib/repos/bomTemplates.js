...
const genC83002RU = (deviceConfig) => {
	...
	if (deviceConfig.device.partNumber === 'C8300-2N2S-4T2X') {
		template = [
			...
			[`DNA-P-T${deviceConfig.tier}-A-3Y`, 'Cisco DNA Advantage On-Prem Lic 3Y', '', 36, 1, 0],
			['SVS-PDNA-ADV', 'Embedded Support for SW - Tiered  DNA Advantage On-Prem', '', 36, 1, 0],
			[`DSTACK-T${deviceConfig.tier}-A`, 'Cisco DNA Advantage Stack', '', 36, 1, 0],
			[`NWSTACK-T${deviceConfig.tier}-A`, 'Cisco Network Advantage Stack', '', 36, 1, 0],
			['SDWAN-UMB-ADV', 'Cisco Umbrella for DNA Advantage', '', 36, 1, 0],
			['DNAC-ONPREM-PF', 'Cisco DNA Center On Prem Deployment Option for WAN', '', 36, 1, 0]
		];
	...
	let updatedTemplate = template.filter((item) => item[4] !== 0);
...
const genC8200 = (deviceConfig) => {
	...
	if (deviceConfig.device.partNumber === 'C8200L-1N-4T') {
		...
		template = [
			...
			['C8200-NIM-BLANK', 'Cisco Catalyst 8200 Edge NIM Blank', '', '', nimBlankSlot, ''],
			[deviceConfig.nimOne.partNumber, deviceConfig.nimOne.description, '', '', nimQty, ''],
			['C8200-PIM-BLANK', 'Cisco Catalyst 8200 Edge PIM Blank', '', '', pimBlankSlots, ''],
			[deviceConfig.pim.partNumber, deviceConfig.pim.description, '', '', pimQty, ''],
			['TE-R-SW', 'TE agent for IOSXE on Enterprise Routing', '', '', 1, ''],
